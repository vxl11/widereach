...
Given a multiset $X$, we denote as $\vu_X(\vect{x})$ the multiplicity of $\vect{x}$ in $X$. 
Given a hyperplane $h = (\vect{w}, c)$ $ in $\R^n$ 
specified by a normal vector $\vect{w} = (w_1 ~ w_2 \dots w_n)^T \in \Q^n$ and a bias $c$,
...
that are consistent with the given hyperplane \cite{mhc}.
